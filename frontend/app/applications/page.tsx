...
import { userApplications } from "@/lib/data";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Application2 } from "@/lib/data";

export default function ApplicationsPage() {
  const [applications] = useState<Application2[]>(userApplications);

  const getApplicationsByStatus = (status: Application2["status"]) => {
    return applications.filter((app) => app.status === status);
  };

  const statuses: Application2["status"][] = [
    "applied",
    "shortlisted",
    "interviewed",
    "rejected",
            ...
            <Tabs defaultValue="Applied" className="w-full">
              <TabsList className="grid w-full grid-cols-4">
                ...
                <TabsTrigger value="Interview">
                  Interview ({getApplicationsByStatus("interviewed").length})
                </TabsTrigger>
